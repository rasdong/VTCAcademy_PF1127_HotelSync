...
namespace HotelManagementSystem.ConsoleUI
{
    public class InvoiceManagementUI : BaseUI
    {
        ...
        private void ViewAllInvoices()
        {
            ...
            try
            {
                invoices = _invoiceBL.GetAllInvoices();
            }
            catch (Exception ex)
            {
                ShowErrorMessage($"Lỗi: {ex.Message}");
            ...
            string[] headers = new[] { "ID", "Booking ID", "Tổng tiền", "Trạng thái", "Ngày tạo" };
            int[] columnWidths = new[] { 8, 12, 15, 15, 12 };
            ...
            int maxRowsToShow = Math.Min(invoices.Rows.Count, 10); // Giới hạn hiển thị 10 dòng
            for (int i = 0; i < maxRowsToShow; i++)
            {
                ...
                string totalAmount = invoices.Rows[i]["TotalAmount"] != DBNull.Value ? 
                    Convert.ToDecimal(invoices.Rows[i]["TotalAmount"]).ToString("N0").PadRight(columnWidths[2]) : "".PadRight(columnWidths[2]);
                string paymentStatus = (invoices.Rows[i]["PaymentStatus"]?.ToString() ?? "").PadRight(columnWidths[3]);
                string issueDate = invoices.Rows[i]["IssueDate"] != DBNull.Value ? 
                    Convert.ToDateTime(invoices.Rows[i]["IssueDate"]).ToString("yyyy-MM-dd") : "";
            ...
            if (invoices.Rows.Count > 10)
            {
                ...
                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.Write($"Hiển thị {maxRowsToShow}/{invoices.Rows.Count} hóa đơn. Sử dụng tìm kiếm để xem chi tiết.");
                Console.ResetColor();
            }
