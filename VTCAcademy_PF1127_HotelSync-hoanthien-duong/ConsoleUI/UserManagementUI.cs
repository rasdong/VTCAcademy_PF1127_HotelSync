...
namespace HotelManagementSystem
{
    public class UserManagementUI : BaseUI
    {
        ...
        private void ShowUserList()
        {
            ...
            int[] columnWidths = new[] { 5, 15, 12, 20, 20, 10, 12 };
            ...
            int maxRowsToShow = Math.Min(users.Rows.Count, 10); // Giới hạn hiển thị 10 dòng
            for (int i = 0; i < maxRowsToShow; i++)
            {
                Console.SetCursorPosition(x + 2, y + 8 + i * 2);
                
                string id = (users.Rows[i]["UserID"]?.ToString() ?? "").PadRight(columnWidths[0]);
                string username = (users.Rows[i]["Username"]?.ToString() ?? "").PadRight(columnWidths[1]);
                string roleName = (users.Rows[i]["RoleName"]?.ToString() ?? "").PadRight(columnWidths[2]);
                string email = (users.Rows[i]["Email"]?.ToString() ?? "").PadRight(columnWidths[3]);
                string fullName = (users.Rows[i]["FullName"]?.ToString() ?? "").PadRight(columnWidths[4]);
                string isActive = (users.Rows[i]["IsActive"]?.ToString() == "True" ? "Hoạt động" : "Tạm khóa").PadRight(columnWidths[5]);
                string createdAt = users.Rows[i]["CreatedAt"] != DBNull.Value ? 
                    Convert.ToDateTime(users.Rows[i]["CreatedAt"]).ToString("yyyy-MM-dd") : "";

                // Cắt ngắn nếu quá dài
                if (username.Length > columnWidths[1]) username = username.Substring(0, columnWidths[1] - 3) + "...";
                if (email.Length > columnWidths[3]) email = email.Substring(0, columnWidths[3] - 3) + "...";
                if (fullName.Length > columnWidths[4]) fullName = fullName.Substring(0, columnWidths[4] - 3) + "...";
            ...
            if (users.Rows.Count > 10)
            {
                Console.SetCursorPosition(x + 2, y + 8 + maxRowsToShow * 2);
                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.Write($"Hiển thị {maxRowsToShow}/{users.Rows.Count} người dùng. Tổng cộng {users.Rows.Count} tài khoản.");
                Console.ResetColor();
            }
