    ...
    public class ServiceManagementUI : BaseUI
    {
        ...
        private void ShowServiceList()
        {
            ...
            if (services.Rows.Count == 0)
            {
                ...
                Console.Write("Không có dịch vụ nào trong hệ thống.");
                Console.ResetColor();
                Console.SetCursorPosition(x + 2, y + 8);
            ...
            int maxRowsToShow = Math.Min(services.Rows.Count, 10); // Giới hạn hiển thị 10 dòng
            for (int i = 0; i < maxRowsToShow; i++)
            {
                Console.SetCursorPosition(x + 2, y + 8 + i * 2);
                
                string id = (services.Rows[i]["ServiceID"]?.ToString() ?? "").PadRight(columnWidths[0]);
                string serviceName = (services.Rows[i]["ServiceName"]?.ToString() ?? "").PadRight(columnWidths[1]);
                string type = (services.Rows[i]["Type"]?.ToString() ?? "").PadRight(columnWidths[2]);
                string price = Convert.ToDecimal(services.Rows[i]["Price"]).ToString("N0").PadRight(columnWidths[3]);
                string createdAt = services.Rows[i]["CreatedAt"] != DBNull.Value ? 
                    Convert.ToDateTime(services.Rows[i]["CreatedAt"]).ToString("yyyy-MM-dd") : "";

                // Cắt ngắn nếu quá dài
                if (serviceName.Length > columnWidths[1]) serviceName = serviceName.Substring(0, columnWidths[1] - 3) + "...";
            ...
            if (services.Rows.Count > 10)
            {
                Console.SetCursorPosition(x + 2, y + 8 + maxRowsToShow * 2);
                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.Write($"Hiển thị {maxRowsToShow}/{services.Rows.Count} dịch vụ. Sử dụng tìm kiếm để xem chi tiết.");
                Console.ResetColor();
            }
        ...
        private void ShowSearchServicesByType()
        {
            ...
            Console.SetCursorPosition(x + 2, y + 5);
            Console.Write("Loại dịch vụ (Food/Laundry/Spa/Other): ");
            string? type = ReadInputWithEscape(x + 39, y + 5);
            if (type == null) return;

            try
            {
                DataTable serviceResult = _serviceBLL.SearchServicesByType(type);
                ShowServiceSearchResults(serviceResult, $"Kết Quả Tìm Kiếm Dịch Vụ - Loại: {type}");
            }
            catch (Exception ex)
            {
                ShowErrorMessage($"Lỗi: {ex.Message}");
        ...
        private void ShowServiceSearchResults(DataTable services, string searchTitle)
        {
            Console.Clear();
            ...
            if (services.Rows.Count == 0)
            {
                Console.SetCursorPosition(x + 2, y + 6);
                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.Write("Không tìm thấy dịch vụ nào phù hợp với tiêu chí tìm kiếm.");
                Console.ResetColor();
                Console.SetCursorPosition(x + 2, y + 8);
                Console.WriteLine("Nhấn phím bất kỳ để quay lại...");
                Console.ReadKey();
            ...
            if (services.Rows.Count > 10)
            {
                Console.SetCursorPosition(x + 2, y + 8 + maxRowsToShow * 2);
                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.Write($"Hiển thị {maxRowsToShow}/{services.Rows.Count} dịch vụ. Tìm thấy tổng cộng {services.Rows.Count} kết quả.");
                Console.ResetColor();
            }
